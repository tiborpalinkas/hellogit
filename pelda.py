...
class ChoiceVibra(Frame):
	...
	def __init__(self, boss = None, colo = 'red'):
		Frame.__init__(self)		# a szülőosztály konstruktora
		...
		Scale(self, length = 150, orient = HORIZONTAL, sliderlength = 25,
			label = 'Frekvencia (Hz) :', from_ = 1., to = 9., tickinterval = 2, troughcolor = colo,
			resolution = 0.25, showvalue = 0, command = self.setFrequency).pack(side = LEFT,
			padx = 5, pady = 5)
		Scale(self, length = 150, orient = HORIZONTAL, sliderlength = 15, troughcolor = colo,
			label = 'Fázis (fok) :', from_ = -180, to = 180, tickinterval = 90,
			showvalue = 1, command = self.setPhase).pack(side = LEFT, pady = 5)
		Scale(self, length = 150, orient = HORIZONTAL, sliderlength = 25,
			label = 'Amplitúdó :', from_ = 1, to = 9, tickinterval = 2, troughcolor = colo,
			showvalue = 0, command = self.setAmplitude).pack(side = LEFT, padx = 5, pady = 5)
...
if __name__ == '__main__':
	...
	fra.configure(bd = 4, relief = GROOVE)
	fra.pack(side = TOP)
